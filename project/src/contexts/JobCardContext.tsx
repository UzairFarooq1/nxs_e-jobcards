...
export function JobCardProvider({ children }: { children: React.ReactNode }) {
  ...
  const addJobCard = async (
    ...
    try {
      ...
      // Map camelCase to snake_case for database (matching your schema)
      const dbJobCard = {
        id: newJobCard.id,
        ...
        date_time: new Date(newJobCard.dateTime).toISOString(), // UTC for DB
      ...
      // Map snake_case back to camelCase for local state (matching your schema)
      const mappedData: JobCard = {
        id: data.id,
        hospitalName: data.hospital_name,
        facilitySignature: data.facility_signature || "",
        engineerSignature: newJobCard.engineerSignature || "",
        ...
        dateTime: new Date(data.date_time).toLocaleString("sv-SE").slice(0, 16),
