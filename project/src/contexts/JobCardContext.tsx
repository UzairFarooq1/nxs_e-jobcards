...
export function JobCardProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadJobCards = async () => {
    ...
    try {
      ...
      if (data) {
        // Map snake_case to camelCase (matching actual schema)
        const mappedData: JobCard[] = data.map((item: any) => ({
          id: item.job_card_id, // Use job_card_id as the main ID
          hospitalName: item.hospital_name,
          facilitySignature: item.facility_signature || "",
          engineerSignature: "", // Not in current schema
          ...
          engineerName: "Unknown Engineer", // Not in current schema, would need to join with engineers table
          engineerId: item.engineer_id,
          dateTime: new Date(item.date_time)
            .toLocaleString("sv-SE")
            .slice(0, 16), // Convert UTC to local time
          createdAt: item.created_at,
          status: "completed" as const, // Default since not in schema
          beforeServiceImages: [], // Not in current schema
          afterServiceImages: [], // Not in current schema
          facilityStampImage: "", // Not in current schema
        }));
  ...
  const addJobCard = async (
    ...
    try {
      ...
      // Map camelCase to snake_case for database (matching actual schema)
      const dbJobCard = {
        job_card_id: newJobCard.id, // Use job_card_id column, not id
        ...
        email_sent: false, // Default value for email_sent
        // Note: id, created_at, drive_file_id are auto-generated by database
        // Note: engineer_signature, before_service_images, after_service_images, facility_stamp_image, status columns don't exist in current schema
      };
      ...
      // Map snake_case back to camelCase for local state (matching actual schema)
      const mappedData: JobCard = {
        id: data.job_card_id, // Use job_card_id as the main ID
        ...
        engineerName: newJobCard.engineerName, // Use from original data since not in DB
